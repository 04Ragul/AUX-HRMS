...
namespace HRMS.Infrastructure.Services.Identity
{
    public class UserService : IUserService
    {
        ...
        public async Task<IResult<UserRolesResponse>> GetRolesAsync(string userId)
        {
            ...
            ApplicationUser user = await _userManager!.FindByIdAsync(userId!)!;
            List<ApplicationRole> roles = await _roleManager.Roles.AsNoTracking().ToListAsync();
        ...
        public async Task<string> ExportToExcelAsync(string searchString = "")
        {
            ...
            string result = await _excelService.ExportAsync(users, sheetName: _localizer["Users"],
                mappers: new Dictionary<string, Func<ApplicationUser, object>>
                {
                    ...
                    { _localizer["CreatedOn (Local)"], item => DateTime.SpecifyKind(item!.CreatedOn??DateTime.UtcNow, DateTimeKind.Utc).ToIndianStandardTime().ToString("G", CultureInfo.CurrentCulture) },
        ...
        public async Task<IResult<string>> ReleaseDevice(string userId)
        {
            ...
            if (usr == null)
            {
                return await Result<string>.FailAsync("User Not Exists");
            }
            var res = await _userManager.UpdateAsync(usr);
            if (res.Succeeded)
            {
                return await Result<string>.SuccessAsync(data: "Device Released");
            }
            return await Result<string>.FailAsync("Not able to Release Device");
        }
        ...
        public async Task<Result<int>> DeleteUser(int userId)
        {
            ...
            if (usr != null)
            {
                ...
                if (res.Succeeded)
                {
                    return await Result<int>.SuccessAsync($"User {usr.UserName} Deleted Permently");
                }
                else
                {
                    return await Result<int>.FailAsync($"Code: {res.Errors.FirstOrDefault()!.Code} , Message {res.Errors.FirstOrDefault()!.Description}");
                }
               
            }
            return await Result<int>.FailAsync("User Not Exists");
        }

        public async Task<IResult<UserResponse>> GetByNameAsync(string userName)
        {
